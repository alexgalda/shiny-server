...
#  })
  ...
  output$n.qubits             <- renderText({paste0("<b>Qubits: <font color='green'>", backend.qubits(), "</font> </b>")})
  output$backend.status       <- renderText({paste0("<b>Status: <font color=", ifelse(backend.status() == "active", "green", "red"), ">", backend.status(), "</font> </b>")})
...
#      list(src = paste0(input$backend, '.png')) #, width = 200
#    }, deleteFile = FALSE)
    
    output$ui.map <- renderUI({
      fluidRow(
        column(width = 12, align = "center",
               renderImage({
                 list(src = paste0('www/', ifelse(paste0(input$backend, ".png") %in% list.files(paste0(dir, "www")), input$backend, "no_image"), '.png'), width = "100%") 
               }, deleteFile = FALSE)
        )
      )
    })
